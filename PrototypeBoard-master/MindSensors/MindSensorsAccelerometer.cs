...
//
//  $File: MindSensorsAccelerometer.cs $ $Revision: 19 $
//-----------------------------------------------------------------------
...
namespace Microsoft.Robotics.Services.Sample.MindSensors.Accelerometer
{
    ...
    [Description("Provides access to the MindSensors Acceleration Sensor.\n(for use with 'Lego NXT Brick (v2)' service)")]
    [DisplayName("(User) MindSensors Acceleration Sensor")]
    [DssCategory(pxbrick.LegoCategories.NXT)]
    public class MindSensorsAccelerationSensor : DsspServiceBase
    {
        /// <summary>
        /// _state
        /// </summary>
        [InitialStatePartner(Optional = true, ServiceUri = ServicePaths.Store + "/MindSensors.Nxt.Accelerometer.config.xml")]
        ...
        /// _main Port
        /// </summary>
        [ServicePort("/mindsensors/nxt/accelerometer", AllowMultipleInstances=true)]
        ...
        [EmbeddedResource("Microsoft.Robotics.Services.Sample.MindSensors.Transforms.Accelerometer.user.xslt")]
        ...
        /// </summary>
        public MindSensorsAccelerationSensor(DsspServiceCreationPort creationPort) : 
                base(creationPort)
        {
        }
        ...
        public virtual IEnumerator<ITask> ConnectToBrickHandler(ConnectToBrick update)
        {
            ...
            {
                if (fault == null)
                    fault = Fault.FromException(new Exception("Failure Configuring MindSensors Accelerometer on Port " + update.Body.ToString()));
            ...
            attachRequest.PollingCommands = new nxtcmd.NxtCommandSequence(_state.PollingFrequencyMs,
                new I2CReadMindSensorsAccelerationSensor(_state.SensorPort));
            ...
            {
                if (fault == null)
                    fault = Fault.FromException(new Exception("Failure Configuring MindSensors Accelerometer"));
                update.ResponsePort.Post(fault);
                yield break;
            }
            ...
            {
                update.ResponsePort.Post(Fault.FromException(new Exception(string.Format("Failure Configuring MindSensors Acceleration Sensor on port: {0}", update.Body.SensorPort))));
                yield break;
            }
